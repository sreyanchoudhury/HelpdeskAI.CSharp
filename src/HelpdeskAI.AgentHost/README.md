...
- **Guards against content filter failures** — `ContentSafetyGuardChatClient` (inserted after `UseFunctionInvocation()` in both v1 and v2 pipelines) catches Azure OpenAI `content_filter` HTTP 400 errors before they terminate the AG-UI SSE stream. On a filter hit it clears the thread's Redis history so a poisoned conversation cannot keep re-triggering the filter, throws `ContentSafetyException` so MAF emits `RUN_ERROR`, and the frontend `onError` handler resets CopilotKit state and shows the ⚠️ message as the first bubble. Logs a warning with `thread.id` for App Insights investigation.
